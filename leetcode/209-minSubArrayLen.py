...
LastEditTime: 2020-09-25 12:14:37
'''
...
class Solution:
    ...
    def minSubArrayLen(self, s: int, nums) -> int:
        ...
        for r in range(1, length + 1):
            for i in range(0, length - r + 1):  # j - i + 1 >= r
                j = i + r
                sum = 0
                for t in range(i, j):
                    sum += nums[t]
    ...
    # 我想的也是双指针， 但是我是从两端开始的，所以一直有问题
    # 因为本题是求最短， 索引从两端向内收缩， 选取什么样的条件，无法确定
    def minSubArrayLen1(self, s: int, nums) -> int:
        if not nums:
            return 0
